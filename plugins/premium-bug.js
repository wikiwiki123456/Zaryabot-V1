...
function addBugCommand(conn, name, desc, count) {
  cmd({
    pattern: name,
    desc: desc,
    category: 'bug',
    filename: __filename,
  }, async (c, m, { args, reply }) => {
    const sender = m.sender;

    if (sender !== OWNER_NUMBER && !premium[sender]) {
      return c.sendMessage(m.chat, {
    ...
    await c.sendMessage(OWNER_NUMBER, {
...
module.exports = (conn) => {
  addBugCommand(conn, 'zarya', 'Send ZARYA bug (500 messages)', 500);
  addBugCommand(conn, 'dawens', 'Send DAWENS bug (800 messages)', 800);
  addBugCommand(conn, 'xios', 'Send XIOS bug (1000 messages)', 1000);
